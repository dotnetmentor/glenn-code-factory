...
import type { BulkDeleteResponse } from '@/api/queries-commands'
...
export interface BulkDeleteDialogProps {
  open: boolean
  onClose: () => void
  /** "boxes" | "snapshots" — for the title + button labels. */
  resourceKind: 'boxes' | 'snapshots'
  items: BulkDeleteItem[]
  isSubmitting: boolean
  lastResult: BulkDeleteResponse | null
  onConfirm: () => void
}

/**
 * Confirmation modal for the bulk-delete flow. Forces the operator to type
      ...
      <DialogContent>
        <Stack spacing={2.5} sx={{ mt: 1 }}>
          <Typography variant="body2" color="text.secondary">
            This is destructive and irreversible. The Box API will be called
            for each resource. Linked rows will break the runtime they point
            at.
          </Typography>
          ...
          {isSubmitting && (
            <Stack direction="row" spacing={1.5} alignItems="center">
              ...
              <Typography variant="caption" color="text.secondary">
                Deleting {items.length} {resourceKind} via the Box API…
              </Typography>
            </Stack>
          )}
