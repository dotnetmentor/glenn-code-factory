...
import {
  useGetApiAdminRuntimesRuntimeIdBoxSnapshot,
  useGetApiProjectsProjectIdBranchesBranchIdRuntimeStatus,
  type BoxOperationView,
  type BoxSnapshotResponse,
...
/**
 * Map a raw Box operation status string onto a semantic dot color. These come
...
export interface BoxTabProps {
   ...
   * (the root cause of the "DB / Box / UI show different things" report).
   */
  branchId: string
 ...
 * canonical "our view ↔ Box's view" comparison plus a list of recent
 * Box API operations so an operator can eyeball drift and the last few
 ...
 * on — which meant the Box tab showed the snapshot of a sibling branch's
 ...
 */
export function BoxTab({ projectId, branchId, active, isSuperAdmin }: BoxTabProps) {
  ...
  const snapshotQuery = useGetApiAdminRuntimesRuntimeIdBoxSnapshot(runtimeId ?? '', {
    query: {
  ...
  if (snapshotQuery.isLoading && !snapshotQuery.data) {
    return <CenteredHint label="Loading box snapshot…" />
  }
      ...
      <Tooltip title="Refresh" enterDelay={400}>
        <span>
          <IconButton
            ...
            onClick={onRefresh}
            disabled={isFetching}
            aria-label="Refresh box snapshot"
            sx={{
              color: workspaceText.muted,
              p: 0.625,
              '&:hover': {
...
interface ComparisonSectionProps {
  data: BoxSnapshotResponse
}

/**
 * Map ourView.state ↔ boxView.status pairs that should be considered in
...
const COMPATIBLE_STATE_PAIRS: ReadonlyArray<readonly [string, string]> = [
  ...
  ['Booting', 'idle'],
  ['Booting', 'running'],
  // Bootstrapping is the post-VM-up, pre-daemon-ready window — the box is
  // fully up on Box's side while our supervisor still does the tarball
  // download + service-up dance. Listed here so the comparison panel doesn't
  // amber-flag the legitimate happy path.
  ['Bootstrapping', 'ready'],
  ['Bootstrapping', 'idle'],
  ['Bootstrapping', 'running'],
  // Waking is the mirror of Bootstrapping for the stop→resume path: Box has
  // already moved the VM back up but the daemon hasn't re-handshaked yet.
  ['Waking', 'provisioning'],
  ['Waking', 'ready'],
  ['Waking', 'idle'],
  ['Waking', 'running'],
  ['Suspended', 'archived'],
  ['Suspending', 'archived'],
  // Suspending + up is the brief window between the DB flip and the Box stop
  // round-trip completing. ArchiveBranchHandler/IdlerJob both issue the stop
  // immediately after the transition and the reconciler retries on drift, so
  // this should be very short-lived — surface as agreement rather than
  // scaring an operator.
  ['Suspending', 'ready'],
  ['Suspending', 'idle'],
  ['Suspending', 'running'],
...
function statesAgree(ours: string | null | undefined, box: string | null | undefined): boolean {
  if (!ours || !box) return true // can't disagree if one side is unknown
  const oursL = ours.toLowerCase()
  const boxL = box.toLowerCase()
  return COMPATIBLE_STATE_PAIRS.some(
    ([o, b]) => o.toLowerCase() === oursL && b.toLowerCase() === boxL,
  )
}

function ComparisonSection({ data }: ComparisonSectionProps) {
  const { ourView, boxView } = data
  // Defensive null check — Swashbuckle quirk makes Orval type boxView as
  // non-null even though the backend returns null when the box isn't found.
  const boxPresent = boxView != null

  const stateDrift = boxPresent && !statesAgree(ourView.state, boxView.status)
  const regionDrift = boxPresent && !!ourView.region && !!boxView.region &&
    ourView.region.toLowerCase() !== boxView.region.toLowerCase()
  const boxIdMissing = !ourView.boxId && ourView.state !== 'Pending'

  return (
    <Box>
      <SectionHeader icon={CompareArrowsOutlinedIcon} label="Our view vs Box" />
      <Box
        sx={{
          mt: 1,
          border: 1,
        ...
        {boxPresent ? (
          <>
            ...
            <CompareRow
              label="Region"
              ours={ourView.region}
              box={boxView.region ?? null}
              drift={regionDrift}
            />
            <CompareRow
              label="Last update"
              ours={formatIso(ourView.stateChangedAt)}
              box={formatIso(boxView.createdAt)}
              mono
            />
            <CompareRow
              label="Machine ID"
              ours={ourView.boxId ?? DASH}
              box={boxView.id}
              drift={
                !!ourView.boxId &&
                ourView.boxId.toLowerCase() !== boxView.id.toLowerCase()
              }
              mono
              last
            />
          ...
          <>
            ...
            <CompareRow
              ...
              ours={ourView.boxId ?? DASH}
              box={null}
              drift={boxIdMissing || !!ourView.boxId}
              boxMissing
              mono
              last
            />
            ...
            >
              ...
              >
                {ourView.boxId ? (
                  <>
                    (box ID <IdChip>{ourView.boxId}</IdChip> no longer exists on Box)
                  </>
                ) : (
                  '(never provisioned)'
                )}
...
function CompareRow({ label, ours, box, drift, boxMissing, mono, last }: CompareRowProps) {
  return (
    <Box
      sx={{
        display: 'grid',
